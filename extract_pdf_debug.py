...
DEBUG_FILE = "pdf_extract_debug.txt"
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    ...
                    if len(debug_lines) < 100:
                        debug_lines.append(f"行{i}: {line}")

                    # 匹配格式：数字. 单词 词性 释义
                    # 例如：1 jeopardize v. 危及；损害
                    match = re.match(r'^(\d+)\.\s+([a-zA-Z\-]+)\s+([a-z]+\.)\s+(.+)', line)

                    if match and len(debug_lines) < 200:
                        word = match.group(2).lower().strip()
                        debug_lines.append(f"  ✓ 匹配到: {word}")

                    # 检查单词是否在列表中
                    if match:
                        word = match.group(2).lower().strip()
                        pos = match.group(3)
                        meaning_and_example = match.group(4)
                        ...
                        if meaning_match:
                            ...
                            meaning = meaning_and_example[:50].strip()
