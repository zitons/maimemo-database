...
def get_review_history_combined(momo_db='momo.v5_5_65.db'):
    ...
    for row in cursor.fetchall():
        ...
        # 完整的4级评分映射
        parse_voc_list(forget, 3)        # forget -> 响应3 -> 将映射为Again
        parse_voc_list(uncertain, 2)     # uncertain -> 响应2 -> 将映射为Hard
        parse_voc_list(familiar, 1)      # familiar -> 响应1 -> 将映射为Good
        parse_voc_list(well_familiar, 4) # well_familiar -> 响应4 -> 将映射为Easy
...
def import_review_history_4level(word_reviews, anki_profile='账户 1'):
    """智能合并导入（完整4级评分版本）"""
    ...
    backup_file = anki_db + f'.backup_4level_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    ...
    total_reviews = 0
    error_count = 0
    ease_stats = {1: 0, 2: 0, 3: 0, 4: 0}  # 统计各评分数量
        ...
        for i, review in enumerate(reviews):
            try:
                ...
                if response == 3:
                    ...
                    ease = 2  # Hard - uncertain
                elif response == 1:
                    ease = 3  # Good - familiar
                elif response == 4:
                    ease = 4  # Easy - well_familiar
                else:
                    ease = 3  # 默认Good
    ...
    print(f"\n评分分布:")
    print(f"  Again: {ease_stats[1]} 条 ({ease_stats[1]*100.0/total_reviews:.1f}%)")
    print(f"  Hard: {ease_stats[2]} 条 ({ease_stats[2]*100.0/total_reviews:.1f}%)")
    print(f"  Good: {ease_stats[3]} 条 ({ease_stats[3]*100.0/total_reviews:.1f}%)")
    print(f"  Easy: {ease_stats[4]} 条 ({ease_stats[4]*100.0/total_reviews:.1f}%)")
    ...
    for card_id in momo_card_ids:
        try:
            ...
            if updated_count % 100 == 0:
                print(f"进度: {updated_count}/{len(momo_card_ids)} 张卡片...")
...
def main():
    print("="*60)
    print("智能合并导入墨墨记忆数据（完整4级评分）")
    print("评分映射: forget→Again, uncertain→Hard, familiar→Good, well_familiar→Easy")
    print("="*60)

    word_reviews = get_review_history_combined()

    print("\n开始导入...")
    success = import_review_history_4level(word_reviews)

    if success:
        print("\n[完成] 数据已成功导入！")
        print("现在数据包含完整的4级评分，FSRS可以更好地优化。")
